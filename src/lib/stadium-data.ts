...
export function simulateCrowdUpdate(zones: Zone[]): Zone[] {
  return zones.map(z => {
    ...
    return { ...z, crowd: Math.max(5, Math.min(95, z.crowd + delta)) };
  });
}

export const foodStalls: FoodStall[] = [
  {
    id: "stall-1", name: "Burger Barn", zoneId: "food-1",
    waitTime: 8, crowd: "medium",
    menu: [{ name: "Classic Burger", price: 9.99 }, { name: "Fries", price: 4.99 }, { name: "Soda", price: 2.99 }],
  },
  {
    id: "stall-2", name: "Pizza Palace", zoneId: "food-2",
    waitTime: 15, crowd: "high",
    menu: [{ name: "Margherita Slice", price: 6.99 }, { name: "Pepperoni Slice", price: 7.99 }, { name: "Water", price: 1.99 }],
  },
  {
    id: "stall-3", name: "Taco Stand", zoneId: "food-3",
    waitTime: 5, crowd: "low",
    menu: [{ name: "Beef Taco", price: 5.99 }, { name: "Nachos", price: 7.49 }, { name: "Lemonade", price: 3.49 }],
  },
];
...
export function generateAlerts(zones: Zone[]): Alert[] {
  ...
  const now = Date.now();
  zones.forEach(z => {
    if (z.type === "gate" && z.crowd > 70) {
      alerts.push({ id: `alert-${z.id}`, message: `${z.name} is overcrowded — consider alternative gates`, severity: "danger", timestamp: now });
    } else if (z.type === "gate" && z.crowd < 35) {
      alerts.push({ id: `hint-${z.id}`, message: `Use ${z.name} for faster entry`, severity: "info", timestamp: now });
    }
    if (z.type === "food" && z.crowd > 65) {
      alerts.push({ id: `food-${z.id}`, message: `${z.name} has long queues — try another stall`, severity: "warning", timestamp: now });
    }
  });
...
export function findPath(zones: Zone[], sourceId: string, destId: string, avoidCrowds: boolean): string[] {
  const zoneMap = new Map(zones.map(z => [z.id, z]));
  const dist = new Map<string, number>();
  const prev = new Map<string, string | null>();
  const visited = new Set<string>();

  zones.forEach(z => { dist.set(z.id, Infinity); prev.set(z.id, null); });
  dist.set(sourceId, 0);
  ...
  for (let i = 0; i < zones.length; i++) {
    ...
    for (const nId of zone.connections) {
      ...
      const weight = avoidCrowds ? 1 + neighbor.crowd / 20 : 1;
